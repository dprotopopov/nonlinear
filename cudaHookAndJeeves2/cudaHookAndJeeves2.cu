...
int main(int argc, char* argv[])
{
	...
	size_t mMax = thrust::max(md, thrust::reduce(m.begin(), m.end(), (size_t)0, max_functor<size_t>()));
	...
	for (unsigned s = 0; s < count; s++)
	{
		...
		while (true)
		{
			...
			if (trace_mode == TRACE && count == 1) std::cout << "kernel0" << std::endl;
			kernel0 <<< blocks , threads >>> (vPtr , tPtr , xPtr , yPtr , ePtr , mPtr , aPtr , bPtr , fPtr , wPtr , total , n , f.size() / (n + 1) , w.size() / (n + 1));
		...
		while (true)
		{
			...
			for (size_t k = 0; k < n; k++)
			{
				...
				while (true)
				{
					if (trace_mode == TRACE && count == 1) std::cout << "kernel1" << std::endl;
					kernel1 <<< blocks , threads >>> (xPtr0 , tPtr , xPtr , yPtr , ePtr , aPtr , bPtr , fPtr , wPtr , ak , bk , k , mk , n , f.size() / (n + 1) , w.size() / (n + 1));
					thrust::copy(yArray.begin(), yArray.end(), hyArray.begin());
			...
			while (true)
			{
				if (trace_mode == TRACE && count == 1) std::cout << "kernel2" << std::endl;
				kernel2 <<< blocks , threads >>> (tPtr , xPtr , yPtr , pPtr , ePtr , xPtr1 , xPtr2 , aPtr , bPtr , fPtr , wPtr , l , h , md , n , f.size() / (n + 1) , w.size() / (n + 1));
				thrust::copy(yArray.begin(), yArray.end(), hyArray.begin());
