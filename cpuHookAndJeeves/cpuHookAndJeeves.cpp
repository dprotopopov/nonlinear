...
int main(int argc, char* argv[])
{
	...
	std::cout << "Число сегментов          : "; for(unsigned i=0;i<m.size();i++) std::cout << m[i] << " "; std::cout << "+ " << md; std::cout << std::endl; 
	...
	while(true)
	{
		...
		for(unsigned k=0; k<n; k++)
		{
			...
			while(true)
			{
				...
				for(unsigned index=0; index<=mk; index++)
				{
		...
		while(true)
		{
			...
			for(unsigned index=-md;index<=md;index++)
			{
			...
			double ll = l;
			double hh = h;
			l=std::max(ll,p-(hh-ll)/md);
			h=std::min(hh,p+(hh-ll)/md);
		}
