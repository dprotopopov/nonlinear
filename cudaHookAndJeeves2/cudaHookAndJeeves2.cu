...
int main(int argc, char* argv[])
{
		...
		while(true)
		{
			...
			for(size_t k=0; k<n; k++)
			{
				...
				while(true)
				{
					...
					kernel1<<< blocks, threads >>>(xPtr0, tPtr, xPtr, yPtr, ePtr, aPtr, bPtr, ak, bk, k, mk, n);
					thrust::copy(yArray.begin(), yArray.end(), hyArray.begin());
			...
			while(true)
			{
				...
				kernel2<<< blocks, threads >>>(tPtr, xPtr, yPtr, pPtr, ePtr, xPtr1, xPtr2, aPtr, bPtr, l, h, md, n);
				thrust::copy(yArray.begin(), yArray.end(), hyArray.begin());
	...
	std::cout << "Число сегментов          : "; for(size_t i=0;i<hm.size();i++) std::cout << hm[i] << " "; std::cout << "+ " << md; std::cout << std::endl; 
